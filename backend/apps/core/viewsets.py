...
class RbacOnlyQuerysetMixin:
    """Opt a tenant-wide resource out of ABAC scope narrowing.

    For catalogues every member may list — units of measure, tax brackets,
    salary components, report definitions — that carry no per-actor scope.
    Tenancy and Row-Level Security still apply; only the actor-scope ``Q`` is
    skipped (see :attr:`ScopedQuerysetMixin.abac`). Mix it in to the left of the
    viewset base. This one declaration replaced seven byte-identical copies that
    each re-implemented ``get_queryset`` to achieve the same thing.
    """
...
class IdempotentActionMixin:
    ...
    For hand-written ``@action`` transitions whose service does not take the
    ``(pk, **kwargs)`` shape :func:`transition_action` expects. It reuses the
    same cache helpers, so a hand-written transition behaves identically to a
    generated one — same header, same cache namespace, same
    ``Idempotency-Replayed`` response header.

    This lived in ``apps.accounting.viewsets`` and was imported by five sibling
    apps; it has nothing to do with the ledger, so it lives here with the rest
    of the shared viewset behaviour.
    """
