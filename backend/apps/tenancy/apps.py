...
dependency graph and every ``"tenancy.Model"`` string reference in the project.
...
class TenancyConfig(AppConfig):
    """Tenants, custom domains, subscriptions and the audit log.

    Loaded before every business app: ``TenantScopedModel.tenant`` is a FK to
    ``tenancy.Tenant``, so the app registry must be able to resolve that
    string reference. It also owns the row-level-security migration, which
    must run before any table it protects exists.
    """
